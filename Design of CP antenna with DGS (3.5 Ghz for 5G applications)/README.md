...
Research project | VIT Vellore, School of Electronics Engineering
...
Circular polarisation is achieved using the truncated corner method, which excites two orthogonal modes with a 90° phase difference. The DGS is etched into the ground plane to improve the Axial Ratio Bandwidth (ARBW) and overall impedance matching. The antenna is fabricated on an FR4 substrate (εr = 4.3, loss tangent ≈ 0.025).
...
DGS improved ARBW by 36.5% at the cost of a slight reduction in impedance bandwidth and gain — a well-understood tradeoff in CP antenna design.

---

## Design Methodology

The design was developed across three progressive stages.

### Stage 1 — Basic Microstrip Patch Antenna

A rectangular microstrip patch designed for 3.5 GHz resonance using standard design equations for patch width, effective dielectric constant, and length extension.

Dimensions:
- Substrate: 35.9 mm x 29 mm
- Patch: 26.3 mm x 19.4 mm
- Feed Width: 3.08 mm
- Substrate Height: 1.6 mm

### Stage 2 — Truncated Corner CP Antenna

Corner truncation applied to excite two orthogonal modes with a 90° phase difference, generating Right Hand Circular Polarisation (RHCP).

Truncation length: t = 0.17 x Lp = 3.55 mm

Dimensions:
- Substrate: 29.6 mm x 30.5 mm
- Patch: 20 mm x 20.9 mm
- Feed Width: 1.6 mm
...
├── Results/            # S11, VSWR, Axial Ratio and Gain plots
│   ├── Stage1_BasicPatch/
│   ├── Stage2_CP_Antenna/
│   └── Stage3_DGS/
├── Images/             # Design views, surface current distributions, 3D gain plots
...
- FR4 Substrate — εr = 4.3, h = 1.6 mm (Stage 1 and 3), h = 3.6 mm (Stage 2)
